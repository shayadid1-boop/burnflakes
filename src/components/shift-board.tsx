...
function fmtDay(d: string) { const dt = new Date(d + "T00:00:00"); return `יום ${DAY_HE[dt.getDay()]} ${dt.getDate()}.${dt.getMonth() + 1}`; }

/** The camp-wide shift board (or one department's, when departmentId is given). Attending members self-register. */
export async function ShiftBoard({ eventId, myEmId, departmentId, back, canRegister = true }: { eventId: string; myEmId: string | null; departmentId?: string; back: string; canRegister?: boolean }) {
  ...
  if (rows.length === 0) return <p className="text-sm text-stone-500">עדיין לא נפתחו משמרות.</p>;

  return (
    <div className="space-y-4">
      {days.map((day) => (
        <div key={day}>
          <h3 className="mb-1 font-semibold">{fmtDay(day)}</h3>
              ...
              return (
                <div key={r.id} className={`rounded-lg border p-3 text-sm ${mine ? "border-orange-300 bg-orange-50" : full ? "border-stone-200 bg-stone-50" : "border-stone-200 bg-white"}`}>
                  <div className="flex items-baseline justify-between">
                    <span className="font-medium">{name}{!departmentId && <span className="text-xs text-stone-500"> · {r.departments?.name_he}</span>}</span>
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    ...
                    {canRegister && myEmId && (mine ? (
                      <form action={unregisterShift}><input type="hidden" name="shift_id" value={r.id} /><input type="hidden" name="back" value={back} /><button className="rounded-md border border-stone-300 bg-white px-2 py-0.5 text-xs">בטל הרשמה</button></form>
                    ) : !full ? (
                      <form action={registerShift}><input type="hidden" name="shift_id" value={r.id} /><input type="hidden" name="back" value={back} /><button className="rounded-md bg-orange-600 px-2 py-0.5 text-xs text-white">הרשם</button></form>
